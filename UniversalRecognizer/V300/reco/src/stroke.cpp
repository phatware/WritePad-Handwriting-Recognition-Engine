...
static _BOOL is_umlyut(p_SPECL cur, _RECT stroke_box, _INT ibeg_str, _INT iend_str,
                       p_SHORT x, p_SHORT y, _INT med_ampl, _INT lang );
static _BOOL	is_t_min(p_SPECL min,p_SHORT x,p_SHORT y,_RECT stroke_box,_INT thresh,
...
_SHORT transfrmN(low_type _PTR D)
{
    ...
    _SHORT	shift = 0;
    _INT	should, width, step_of_wr=-1,apr_height,shtraf,shtraf0=0;
    _INT	unsureness = 0;
    _INT	i, n, cur_i, cur_y, np;
    _INT	med_ampl=0, mid_ampl=0, max_ampl=0, max_dist=0, max_up_height=0, mid_dev = 1;
    _LONG	dev_abs_sum;
    _INT	med_height,med_u_bord,med_d_bord,t_height=0,ft_height=0;
    _INT	n_allmax = 0, n_allmin = 0, n_ampl = 0, n_line_max=0, n_line_min = 0;
    _INT    n_line_max0 = 0, n_line_min0 = 0, n_x, n_strokes = 0;
    _INT	dyAll = D->box.bottom-D->box.top;
    _INT	UP_LINE_POS = 0, DOWN_LINE_POS = 0;
    _INT	x_left_max = 0, x_right_max = 0, x_left_min = 0, x_right_min = 0;
    _INT	CONSTT;
    p_SHORT ampl =_NULL, bord_u, bord_d;
    p_SHORT x = D->buffers[0].ptr, y=D->buffers[1].ptr;
    p_SHORT i_back = D->buffers[2].ptr;
    p_SHORT i_point = D->buffers[3].ptr;
    p_EXTR	line_max = _NULL, line_min = _NULL, line_max1 = _NULL;
    p_EXTR  line_min1 = _NULL, line_max0 = _NULL, line_min0 = _NULL;
    p_SPECL cur;
    _BOOL	del = _FALSE, ins=_FALSE, change_min=_FALSE, change_max=_FALSE, defis;
    _BOOL   sep_let = _FALSE, nonpos_height, gl_down_left=_FALSE, gl_up_left = _FALSE;
    _UCHAR	pass=0;
    
    if (D->rc->rec_mode!=RECM_FORMULA  && (D->rc->low_mode & LMOD_BORDER_TEXT) )
        ...
        && D->rc->lmod_border_used==LMOD_BORDER_TEXT)
    {
        ...
        n_strokes= classify_strokes(D,med_ampl,max_ampl,n_ampl,&t_height,&ft_height,&sep_let);
    ...
    mem_segm = HWRMemoryAlloc(4L*MAX_NUM_EXTR*sizeof(EXTR) + 2L*D->ii*sizeof(_SHORT));
    if (mem_segm==_NULL)
        goto EXIT_FREE;
    ...
    {
        unsureness+=100;
        SpecBord(D,bord_d,bord_u,&med_d_bord,&med_u_bord,&med_height, &n_x,0,line_max,0);
        goto TRANSFORM;
    }
...
BEGIN:
    ...
    if (D->rc->lmod_border_used==LMOD_BORDER_NUMBER)
    {
        ...
        num_bord_correction(line_max,&n_line_max,n_allmax,MAXW,med_ampl,bord_d,y, D->rc->lang );
        goto BORD_D;
    }
    
    if (extract_all_extr(D,MAXW,line_max,&n_allmax,&n_line_max,&shift) == UNSUCCESS)
        goto EXIT_FREE;
    //  n_line_max=n_allmax;
    if (extract_all_extr(D,MINW,line_min,&n_allmin,&n_line_min,&shift) == UNSUCCESS)
        goto EXIT_FREE;
    //  n_line_min=n_allmin;
    if (n_line_min==0 || n_line_max==0)
    {
...
BORD_D:
    ...
    if (D->rc->lmod_border_used==LMOD_BORDER_NUMBER)
    {
        del=num_bord_correction(line_min,&n_line_min,n_allmin,MINW,med_ampl,bord_d,y, D->rc->lang );
        if (del==_TRUE)
        {
            del=num_bord_correction(line_max,&n_line_max,n_allmax,MAXW,med_ampl,bord_d,y, D->rc->lang );
            if (del==_TRUE)
            {
                smooth_d_bord(line_max,n_line_max,D,should,bord_d);
            }
    ...
    if (del==_TRUE)
    {
        bord_correction(D,line_min,&n_line_min,n_allmin,MINW,
...
BORD_U:
    ...
    if (pass==1 && shtraf>=3 && shtraf0>=3 &&  (n_line_min<=1||n_line_max<=1) && (n_line_min0<=1||n_line_max0<=1) )
    {
        unsureness+=100;
        SpecBord(D,bord_d,bord_u,&med_d_bord,&med_u_bord,&med_height,
                 &n_x,0,line_max,n_line_max);
    ...
    if (lang == LANGUAGE_GERMAN)
        ...
        CONSTT=2;
    if (CONSTT*(bord_d[i_point[0]]-bord_u[i_point[0]])<=med_height ) //|| bord_d[i_point[0]]-bord_u[i_point[0]]>=CONSTT*med_height
    {
        unsureness+=5;
    ...
    if (CONSTT*(bord_d[i_point[n_x-1]]-bord_u[i_point[n_x-1]])<=med_height ) // || bord_d[i_point[n_x-1]]-bord_u[i_point[n_x-1]]>=CONSTT*med_height
    {
        unsureness+=5;
        if (n_line_min<1 || n_line_max<1) goto TRANSFORM;
    ...
    {
        if (calc_med_heights(D,line_min,line_max,bord_u,bord_d,i_point,n_line_min,
                             n_line_max,n_x,&med_height,&med_u_bord,&med_d_bord) != SUCCESS)
            goto EXIT_FREE;
    }
...
TRANSFORM:
    ...
    if  ( (n_allmax+n_allmin) <= 6 && med_height > THREE_FOURTH(dyAll) && dyAll > 0 )
    {
    ...
    if (D->ii > 0)
    {
        ...
        if  ( FillRCNB(i_point, n_x, D, bord_u, bord_d)
             == UNSUCCESS )
            goto  EXIT_FREE;   /*CHE*/
        ...
        {
            if (D->y[i] != BREAK)
                bord_u[i] = (A*D->x[i] + B)/100 - nu;
        }
        for(i = 0; i < D->ii; i ++)
        {
            if (D->y[i] != BREAK)
                bord_d[i] = (A*D->x[i] + B)/100 + nd;
        }
    }
#endif //0
    /***************************************************************************/
    ...
    /***************************************************************************/
    /*  Fill RC field describing curved border: */
    if  ( FillRCNB(i_point, n_x, D, bord_u, bord_d)  == UNSUCCESS )
    {
        goto  EXIT_FREE;   /*CHE*/
    }
    /***************************************************************************/
                      ...
                      p_INT pn_all_extr, p_INT pn_line_extr, p_SHORT pshift)
{
    ...
    _INT	i = 0, j, i_extr, ibeg_str = 0,iend_str = 0,shift=0,dshift=0;
...
{
    p_SPECL	cur=D->specl,wrk;
    _INT		i=0, i_extr,ibeg_str = 0,iend_str = 0,extr_height;
    ...
    while (cur != _NULL)
    {
        ...
        if (cur->mark==END && (cur->attr==NORM || cur->attr==STRT ||
                      ...
                      _INT n_ampl,p_INT pt_height,p_INT pft_height,p_BOOL psep_let)
{
    ...
    _INT	ibeg_str=0, iend_str=0, prv_ibeg_str=0, prv_iend_str=0, n_str=0;
    _INT    thresh=D->rc->stroka.extr_depth,n_min = 0,n_max = 0,height,tmin;
    _RECT	stroke_box, prv_stroke_box;
    ...
    *psep_let=_TRUE;
    ...
    while (cur!=_NULL)
    {
        ...
        if (cur->mark==MAXW && prv->mark==BEG)
        {
            if ( (prv->prev->prev==_NULL && n_ampl>5
                  && (lang == LANGUAGE_GERMAN || (nxt->next->mark==MAXW && y[nxt->next->ipoint0]>y[cur->ipoint0]))
                  ) ||
                (nxt->mark==MINW && y[cur->ipoint0]-y[nxt->ipoint0] < HWRMax(med_ampl,MIN_STR_HEIGHT)) )
                cur->attr=BEG_MAX;
        }
        ...
        if (cur->mark==MAXW && nxt->mark==END)
        {
            if (prv->mark==MINW)
            {
                if (lang == LANGUAGE_GERMAN)
                {
                    if (y[cur->ipoint0]-y[prv->ipoint0]<MIN_STR_HEIGHT
                ...
                else if (y[cur->ipoint0]-y[prv->ipoint0] <= HWRMax(ONE_EIGHTTH(max_ampl),MIN_STR_HEIGHT))
                {
                    cur->attr=END_MAX;
                }
            }
        ...
        if (cur->mark==MINW && nxt->mark==END)
        {
            ...
            if (nxt->next==_NULL || x[cur->iend]<=D->box.left+thresh || x[cur->iend]>=D->box.right-thresh)
            {
                _INT beg=prv->ipoint0, end=cur->iend;
                _INT iMostFar=iMostFarFromChord(x,y,(_SHORT)beg,(_SHORT)end);
                /*04_14*/   _LONG MaxScal=(x[iMostFar]-x[beg])*(_LONG)(y[beg]-y[end])+
                (y[iMostFar]-y[beg])*(_LONG)(x[end]-x[beg]);
                if (MaxScal>0)
                    cur->attr=END_MIN;
            }
        ...
        if (cur->mark==MAXW && nxt->mark==MINW && nxt->next->mark==MAXW)
        {
                ...
                x[nxt->ibeg]>x[cur->ipoint0] && x[nxt->iend]<x[cur->ipoint0])
            {
                cur->attr=E_MAX;
                if (lang == LANGUAGE_GERMAN &&
                    prv->mark==MINW && prv->prev->mark==BEG &&
        ...
        if (cur->mark==END)
        {
            ...
            if (is_umlyut( cur,stroke_box,ibeg_str,iend_str,x,y,med_ampl, lang )==_TRUE)
            {
                cur->attr=UML;
                wrk=cur;
                while (wrk->mark!=BEG)
            ...
            if (hor_stroke(cur,x,y,n_str)==_TRUE)
            {
                ...
                if (!(D->rc->low_mode & LMOD_SMALL_CAPS))
                {
                    ...
                    {
                        if (wrk->mark==MINW && is_t_min(wrk,x,y,stroke_box,thresh,ibeg_str,iend_str,0,&height)==_TRUE)
                        {
                            wrk->attr=T_MIN;
                            tmin++;
                            *pft_height=HWRMax(*pft_height,height);
                    ...
                    if (tmin==0)
                    {
                        ...
                        while (wrk!=_NULL /*&& wrk->mark!=BEG*/)
                        {
                            if (wrk->mark==MINW && is_t_min(wrk,x,y,stroke_box,thresh,ibeg_str,iend_str,1,&height)==_TRUE)
                            {
                                wrk->attr=T_MIN;
                                *pft_height=HWRMax(*pft_height,height);
                                if (x[wrk->iend]-x[wrk->ibeg]>-thresh)
            ...
            if (cur->prev->mark==BEG && stroke_box.right-stroke_box.left<thresh )
                cur->attr=PNT;
    ...
    epunct=0;
    if (n_str>1)
        epunct=end_punct(D,cur,med_ampl);
    if (epunct!=0)
    {
        wrk=cur;
        wrk->attr=PUNC;
               ...
               _INT ibeg_str,_INT iend_str, _UCHAR pass,p_INT pheight)
{
    ...
    p_SPECL max = (min->next->mark==MAXW) ? min->next : min->prev;
    _INT    max_pnt=max->ipoint0;
    _SHORT  x1=x[min->iend], y1=y[min->ipoint0], x2=x[max->ibeg];
    _SHORT  y2=y[max->ipoint0], x3=x[ibeg_str], y3=y[ibeg_str];
    _SHORT  x4=x[iend_str], y4=y[iend_str];
    ...
    else if (x[min->iend]-x[min->ibeg]<0 &&                     /*case of "f"*/
...
{
    p_SPECL	cur=D->specl,wrk,prv_end = 0;
    _INT	ibeg_str=0,iend_str=0,ibeg_prv = 0,iend_prv = 0,
    dx,dy,iLeft,y_up,y_down,
    com_or_brkt,num_of_str=0,num_of_line_str=0,n_ext,
    thresh=D->rc->stroka.extr_depth;
    _SHORT  x_cross,y_cross,x1=0,y1=0,x2,y2,x3,y3,x4,y4;
    p_SHORT x=D->buffers[0].ptr,
    y=D->buffers[1].ptr;
    _UCHAR	attr=0,prev_attr=0;
        ...
        if (cur->mark==END)
        {
            ...
            if (straight==_TRUE || hor_stroke(cur,x,y,1)==_TRUE)
            {
                ...
                if (attr==STRT && (prev_attr==NORM || prev_attr==L_BRKT))
                {
                    ...
                    {
                        _LONG thresh=(_LONG)(D->rc->stroka.extr_depth);
                        if ( QDistFromChord(x1,y1,x2,y2,x4,y4) <= THREE(thresh*thresh) )
                        {
                            wrk=prv_end;
                            n_ext=0;
            ...
            if (straight==_FALSE)
            {
                ...
                if (com_or_brkt==-1)
                    attr=R_BRKT;
                if (com_or_brkt<0 && prev_attr!=0 &&y1>TWO_THIRD(prv_box.top)+ONE_THIRD(prv_box.bottom))
                    attr=COM;
            }
...
_INT extract_ampl(low_type _PTR D,p_SHORT ampl,p_INT pn_ampl)
{
    ...
    _INT i=0, y_MIN, y_MAX;
    p_SHORT y=D->buffers[1].ptr;
    while (cur != _NULL)
    {
        if (cur->mark==MINW && (cur->attr==NORM || cur->attr==I_MIN || cur->attr==STRT) )
        {
            y_MIN = y[cur->ipoint0];
            prv=cur->prev;
            nxt=cur->next;
...
{
    _INT	i, k, mediana, sign=0;
    _INT    vol_med_level, vol_medNew_level=0, wdif=0, wdifNew=0;
    _INT    n_lower, n_greater;
...
_UCHAR end_punct(low_type _PTR D,p_SPECL cur,_INT med_ampl)
{
    ...
    p_SPECL wrk, prv_end; iend_str=cur->iend; wrk=cur->prev;
    ...
    if (com(D,cur,ibeg_str,iend_str,5)==_TRUE && lst_box.bottom<ONE_HALF(D->box.top)+ONE_HALF(D->box.bottom) )
    {
        if (com(D,prv_end,ibeg_prv_str,iend_prv_str,5)==_TRUE && prv_box.bottom<ONE_HALF(D->box.top)+ONE_HALF(D->box.bottom) )
            return(2);
        else
            return(1);
    }
    
    if (com(D,cur,ibeg_str,iend_str,5)==_TRUE && lst_box.top>TWO_THIRD(D->box.top)+ONE_THIRD(D->box.bottom))
        return(1);
...
{
    p_SHORT x=D->buffers[0].ptr, y=D->buffers[1].ptr;
    _INT	i, dx, dy, pnt1, pnt2, iMostFar, sign = 1, n_segm;
    ...
    wrk=pend->prev;
    if (wrk->mark==BEG || HWRAbs(x[wrk->iend]-x[wrk->ibeg])>TWO_THIRD(HWRAbs(dy)) )
        return(0);
    if (wrk->mark==MINW)
    {
    ...
    if (wrk->mark==BEG)
        return(0);
    if (Q*(x[wrk->ibeg]-x[wrk->iend])>P1*thresh || Q*(x[wrk->iend]-x[wrk->ibeg])>P2*thresh )
        return(0);
    wrk=wrk->prev;
    if (wrk->mark!=BEG)
    {
    ...
    if (C_str*HWRLAbs(MaxScal) <= ChordLenSq) return(0);
    if (MaxScal>0) sign=1;
    if (MaxScal<0) sign=-1;
...
_BOOL str_com(_INT ibeg_str,_INT iend_str,p_SHORT x,p_SHORT y,_INT C_str)
{
    ...
    if (straight_stroke(ibeg_str,iend_str,x,y,C_str)==_TRUE && dy >= THREE_FOURTH(dx))
        return(_TRUE);
    else
        return(_FALSE);
}
...
{
    if (box.right-box.left<ONE_THIRD(med_ampl) && box.bottom-box.top<ONE_THIRD(med_ampl) )
        return(_TRUE);
    else
        return(_FALSE);
}
...
_UCHAR lead_punct(low_type _PTR D)
{
    ...
    if (com(D,fst_end,ibeg_str,iend_str,5)==_TRUE && fst_box.bottom<ONE_HALF(D->box.top)+ONE_HALF(D->box.bottom) )
    {
...
_BOOL straight_stroke(_INT ibeg, _INT iend, p_SHORT x, p_SHORT y,_INT C)
{
    ...
    _LONG   MaxScal = (x[iMostFar]-x[ibeg])*(_LONG)dxNormal+ (y[iMostFar]-y[ibeg])*(_LONG)dyNormal;
...
{
    _INT    dist, min_dist=ALEF, x_mid=ONE_HALF(stroke_box.left)+ONE_HALF(stroke_box.right);
    p_SHORT x=D->buffers[0].ptr, y=D->buffers[1].ptr;
    ...
    {
        _INT MIN_DST=HWRMax(2*D->rc->stroka.extr_depth, ONE_HALF(stroke_box.right-stroke_box.left));
        while (cur->mark!=BEG)
            cur=cur->prev;
        while(cur!=_NULL)
        {
            if (cur->mark==MINW && cur->next->mark!=END)
            {
                if (y[cur->ipoint0] <= stroke_box.bottom || cur->attr==T_MIN ||
                    cur->attr==S_MIN || cur->attr==E_MIN  || !extrs_open(D,cur,MINW,1) )
                {
                    cur=cur->prev;
                    continue;
                }
                       ...
                       p_SHORT x, p_SHORT y,_INT med_ampl, _INT lang )
{
    ...
    if (left_min!=_NULL && y[left_min->ipoint0]>stroke_box.bottom &&
        ...
        && (lang == LANGUAGE_GERMAN || x[nxt_left_min->ipoint0]>stroke_box.right) )
    {
        return(_TRUE);
    }
    if (left_min!=_NULL && y[left_min->ipoint0]>stroke_box.bottom &&
    ...
    if (left_min==_NULL)
    {
        while (cur->mark!=END && cur->mark!=MINW)
            cur=cur->next;
                        ...
                        p_SHORT bord_d,p_SHORT y,_BOOL sl,_BOOL strict, _INT lang )
{
    ...
    if (lang == LANGUAGE_GERMAN)
    {
        tg=TG1_GER[pos][linetype][dir];
        h=H1_GER[pos][linetype][dir];
    }
    else
    {
                            ...
                            _BOOL strict, _INT lang )
{
    ...
    for (shift=1; shift<=MAX_SHIFT; shift++)
    {
        ...
        for (i=shift; i<n_extr; i++)
        {
            ...
            if ((i>shift || type==MAXW) && prev_glitch==_TRUE)
                continue;
            dxl = (i>shift) ? (extr[i-shift].x-extr[i-shift].shift)- (extr[i-shift-1].x-extr[i-shift-1].shift) : 0;
            dyl = (i>shift) ? extr[i-shift].y-extr[i-shift-1].y : 2*med_ampl;
            sgnl = (i>shift) ? sign(dyl,0) : 0;
            dyl = HWRAbs(dyl);
            dxr = (i<n_extr-1) ? (extr[i+1].x-extr[i+1].shift)- (extr[i].x-extr[i].shift) : 0;
            ...
            if (type==MINW)
            {
                ...
                if (i>shift)
                    ampl_l = HWRMin(ampl_l, bord_d[extr[i-shift-1].i]-extr[i-shift-1].y);
                if (i<n_extr-1)
                    ampl_r=HWRMin(ampl_r, bord_d[extr[i+1].i]-extr[i+1].y);
                ampl_l=HWRMin(ampl_l,med_ampl);
                ampl_r=HWRMin(ampl_r,med_ampl);
            ...
            pos=1;
            if (i==shift && extr[0].x<=x_left)
                pos=0;
            if (i==n_extr-1 && extr[n_extr-1].x>=x_right)
                pos=2;
            if (lang == LANGUAGE_GERMAN)
            {
                tg=TG2_GER[pos][linetype][dir];
            }
            else
            {
                        ...
                        _INT mid_ampl,_BOOL gl_down_left)
{
    ...
    for (i=0; i<n_line_max; i++)
    {
        if (line_max[i].susp==GLITCH_DOWN && (line_max[i].pspecl)->code!=RET_ON_LINE)
        {
        ...
        if (line_max[i].susp==DBL_GLITCH_DOWN && i<n_line_max-1 )
        {
            cur=line_max[i].pspecl;
            prv=cur->prev;
            nxt=cur->next;
...
_BOOL non_sub(p_SPECL cur,p_SHORT x,p_SHORT y,_INT thresh)
{
    ...
    if (2*(x[cur->iend]-x[cur->ibeg]) < 5*(y[cur->ipoint0]-HWRMin(y[cur->ibeg],y[cur->iend])) )
    {
        return(_FALSE);
    }
    prv=cur->prev;
...
_VOID glitch_to_inside(p_EXTR extr, _INT n_extr, _UCHAR type, p_SHORT y,
                       _INT mid_ampl,_INT x_left,_INT x_right, _INT lang)
{
    p_SPECL	cur,nxt,prv,wrk,wnxt,wprv;
    _INT	i;
    _UCHAR	MARK = GLITCH_UP, DBL_MARK = DBL_GLITCH_UP, opp_type = MINW;
    ...
    for ( i=0; i<n_extr; i++ )
    {
        if (extr[i].susp==MARK && (extr[i].pspecl)->code!=RET_ON_LINE )
        {
            cur=extr[i].pspecl;
            prv=cur->prev;
            nxt=cur->next;
        ...
        if (extr[i].susp==DBL_MARK && i<n_extr-1 )
        {
            cur=extr[i].pspecl;
            wrk=extr[i+1].pspecl;
            prv=cur->prev;
