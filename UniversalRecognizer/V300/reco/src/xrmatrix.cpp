...
#if !(defined LSTRIP && defined _EMBEDDED_DEVICE)
...
_INT  xrmatr_alloc(p_rc_type rc, p_xrdata_type xrd, p_xrcm_type _PTR xrcm)
{
    ...
    if (rc->corr_mode & XRCM_CACHE)
        alloc_size += cc_size;   /* Allocate cache if requested */
    
    alloc_size += 8;                                         /* DWORD align    */
    
    if ((alloc_addr = HWRMemoryAlloc(alloc_size)) == _NULL)
        goto err;
    ...
    xm->s_inp_line = (p_cline_type)(mem+shift);
    shift += sizeof(_USHORT)*xi_len;
    xm->s_out_line = (p_cline_type)(mem+shift);
    shift += sizeof(_USHORT)*xi_len;
    for (i = 0; i < DTI_MAXVARSPERLET; i ++)
    {
        xm->s_res_lines[i]= (p_cline_type)(mem+shift);
        shift += sizeof(_USHORT)*xi_len;
    }
    xm->p_self_corr = (p_SHORT)(mem+shift);
    shift += sizeof(_USHORT)*xi_len;
    xm->xrinp = (xrinp_type (_PTR)[XRINP_SIZE])(mem+shift);
    shift += sizeof(xrinp_type)*xi_len;
    if (rc->corr_mode & XRCM_CACHE)
    {
        xm->cc_size = cc_size;
        xm->cc = (p_xrcm_cc_type)(mem+shift);
        /* shift += sizeof(xrcm_cc_pos_type)*n_links;*/
    }
...
_INT change_direction(_INT change_to, p_xrcm_type xrcm)
{
    ...
    if (xrcm->inverse != direction)            /* Inverse xrinp if needed */
    {
        ...
        if (xrcm->cc)
            HWRMemSet(xrcm->cc, 0, (_WORD)xrcm->cc_size);
    }
    ...
    if (xrcm->inverse)  // Inverse pass set link info
    {
        ...
        {
            if (IS_XR_LINK(xi->type))
                xrcm->link_index[i-1] = (_CHAR)(j ++);
            else
                xrcm->link_index[i-1] = 0;
        }
...
_INT CountWord(p_UCHAR word, _INT caps_mode, _INT flags, p_xrcm_type xrcm)
{
    ...
    for (l = 0; l < wl; l ++)
    {
        ...
        if (IsAlpha(sym))
        {
            ...
            {
                if (caps_mode & XCM_FL_TRYCAPSp)
                    xrcm->cmode |= XCM_AL_TRYCAPS;
                if (caps_mode & XCM_AL_TRYS4Cp)
                    xrcm->cmode |= XCM_AL_TRYS4C;
                if (!(caps_mode & XCM_FL_DEFSIZEp))
                    xrcm->cmode &= ~(XCM_AL_DEFSIZE);
            }
            else                                               /* Not first letter */
            {
                if (caps_mode & XCM_AL_TRYCAPSp)
                    xrcm->cmode |= XCM_AL_TRYCAPS;
                if (caps_mode & XCM_AL_TRYS4Cp)
                    xrcm->cmode |= XCM_AL_TRYS4C;
                if (!(caps_mode & XCM_AL_DEFSIZEp))
                    xrcm->cmode &= ~(XCM_AL_DEFSIZE);
            }
        }
...
_INT CountLetter(p_xrcm_type xrcm)
{
    ...
    if (IsAlpha(let))
    {
        if ((cmode & XCM_AL_DEFSIZEp) == 0)
            sym = 0;
        if (IsLower(let))                                     /* Smalls */
        {                                                    /* First letter */
            if ((cmode & XCM_AL_TRYCAPSp) != 0)
                altsym = (_UCHAR)ToUpper(let);
        }
        else
        {
            if ((cmode & XCM_AL_TRYS4Cp) != 0)
                altsym = (_UCHAR)ToLower(let);
        }
...
_INT CountSym(p_xrcm_type xrcm)
{
    ...
    sym = (_UCHAR)OSToRec(xrcm->sym, dp->language );
    ...
    if (xrcm->cc && (!(xrcm->flags & (XRMC_DOTRACING | XRMC_CACHESUSPEND))))
    {
        ...
        if ((*xrcm->cc)[loc][sym-DTI_FIRSTSYM].pos)
            cstate = 1;  /* Need not to be counted -- already in cache */
        if (!loc)
            cstate = 0;
    }
    //#endif
    ...
    if (!(cstate & 0x01))
    {
        ...
        if (sfc == _NULL)
            goto err; // Pointer was 0!, symbol not defined
        if (sfc->num_vars == 0)
            goto err;
        ...
        for (v = 0; v < DTI_MAXVARSPERLET; v ++)
        {
            ...
            if (v >= sfc->num_vars)
                count_var = 0;
#if DTI_COMPRESSED
            if (count_var && (len = xrpv->nx_and_vex & DTI_NXR_MASK) < 1)
                count_var = 0;
            if (count_var && (xrpv->veis & (_UCHAR)(xrcm->en_ww << DTI_OFS_WW)) == 0)
                count_var = 0;
            if (count_var && (xrcm->flags & XRMC_RESTRICTEDCAP) && (xrpv->veis & (_UCHAR)(DTI_CAP_BIT)))
                count_var = 0;
            if (!(xrcm->flags & XRMC_USELEARNINF))
                xrcm->vb[v] = (_UCHAR)(xrpv->nx_and_vex >> DTI_VEX_OFFS);
            if (count_var && (xrcm->flags & XRMC_DISABLEON7) && (xrcm->vb[v] & 0x07) == 7)
                count_var = 0;
#else
            if (count_var && (len = sfc->var_lens[v]) < 1)
                count_var = 0;
            if (count_var && (sfc->var_veis[v] & (_UCHAR)(xrcm->en_ww << DTI_OFS_WW)) == 0)
                count_var = 0;
            if (count_var && (xrcm->flags & XRMC_RESTRICTEDCAP) && (sfc->var_veis[v] & (_UCHAR)(DTI_CAP_BIT)))
                count_var = 0;
            if (!(xrcm->flags & XRMC_USELEARNINF))
                xrcm->vb[v] = (_UCHAR)(sfc->var_vexs[v]);
            if (count_var && (xrcm->flags & XRMC_DISABLEON7) && (xrcm->vb[v] & 0x07) == 7)
                count_var = 0;
#endif
            if (count_var && (xrcm->svm & (0x0001 << v)))
                count_var = 0;
...
#if DTI_COMPRESSED
            if (len)
                xrpv = (p_dte_var_header_type)((p_UCHAR)xrpv + sizeof(*xrpv) + (len-1)*sizeof(xrp_type));
#else
            xrpv += len;
#endif
        }
        
        if (MergeVarResults(xrcm))
            goto err;
    }
    ...
    if (cstate)
    {
        ...
        if (cstate == 1) // Restore from cache
        {
            ...
            if (pos > 1)
                (*xrcm->s_out_line)[pos-2] = (_SHORT)(w - 2*XRMC_DEF_CORR_VALUE);
            if (pos > 0)
                (*xrcm->s_out_line)[pos-1] = (_SHORT)(w - XRMC_DEF_CORR_VALUE);
            if (pos < en)
                (*xrcm->s_out_line)[pos+1] = (_SHORT)(w - XRMC_DEF_CORR_VALUE);
            if (pos <=en)
                (*xrcm->s_out_line)[pos+2] = (_SHORT)(w - 2*XRMC_DEF_CORR_VALUE);
        ...
        if (cstate == 2) // Put down in cache
        {
            ...
            if (pos == loc && w >= 0)
                pos = 0; // Do not cache uneven vars
...
_INT CountVar(p_xrcm_type xrcm)
{
    ...
    if (limit > xrcm->xrinp_len)
        limit = xrcm->xrinp_len;
    
#if DTI_COMPRESSED
    if (xrcm->inverse == 0)
    {
        inc = 1;
        xrcm->xrp = xrcm->xrpv->xrs;
    }
    else
    {
        inc = -1;
        xrcm->xrp = xrcm->xrpv->xrs + (len-1);
    }
#else
    if (xrcm->inverse == 0)
    {
    ...
    for (n = 0; n < len; n ++, xrcm->xrp += inc)
    {
        ...
        if (xrcm->inp_start > pos-1)
            xrcm->inp_start = pos-1;
        xrcm->inp_end = xrcm->src_end + n + 1;
        if (xrcm->inp_end > limit)
            xrcm->inp_end = limit;
...
_INT CountXrC(p_xrcm_type xrcm)
{
    ...
    for (; lc < le; lc ++, lp ++, pxrinp ++)
    {
        ...
        if (!((xrp->attr & TAIL_FLAG) && !(pxrinp->attrib & TAIL_FLAG)))
        {
            ...
            if (ct > 0)
            {
                vc  = ppip + ct + ((xhc[pxrinp->height] >> sh_ch) & 0x0F) +
...
_INT MergeVarResults(p_xrcm_type xrcm)
{
    ...
    for (i = st, ol = &(*xrcm->s_out_line)[st]; i < en; i ++)
        *(ol++) = XRMC_FLOOR;
...
_INT MergeWithOutLine(p_SHORT p_line, _INT st, _INT count, p_xrcm_type xrcm)
{
    ...
    if (xrcm->flags & XRMC_DOTRACING)
    {
        ...
        for (i = vst; i < vend; i ++)
        {
            if (i < st)
            {
                mv[i] = 1;
                continue;
            }          // Extr has not started yet
            if (i < mst)
            {
                ol[i] = pl[i-st];
                mv[i] = 0;
                continue;
            }  // Intr has not started yet
    ...
    else
    {
        ...
        {
            if (i < st)
                continue;                       // Extr has not started yet
            if (i < mst)
            {
                ol[i] = pl[i-st];
                continue;
            ...
            if (pl[i-st] >= ol[i])
                ol[i] = pl[i-st];     // Both present -- compare
        }
    }
...
_INT GetFinalWeight(p_xrcm_type xrcm)
{
    ...
    if (xrcm->wwc_pos < xrcm->xrinp_len - XR_SIZE)
        goto err;
#endif
...
err:
    ...
    else
    {
        ...
        for (i = xrcm->v_end; wc > 0 && i < xrcm->xrinp_len; i ++)
            wc -= (*xrcm->xrinp)[i].penalty;
    }
...
_INT TraceAddAlloc(p_xrcm_type xrcm)
{
    ...
    if (pth->cur_block >= XRMC_MAX_TRACE_BL-1)
        goto err;
...
_INT CreateLayout(p_xrcm_type xrcm)
{
    ...
    if (xrcm->p_htrace == _NULL)
        goto err;
    
    if (xrcm->p_hlayout)
        HWRMemoryFree(xrcm->p_hlayout->mem);
    ...
    for (l = wl-1; l >= 0; l--)
    {
        ...
        if (tp < plh->v_start || tp >= plh->v_end)
            goto err;
        ...
        if (tp < psh->v_start || tp >= psh->v_end)
            goto err;
        nvar = psh->merge_vect[tp];
        
        if (nvar >= DTI_MAXVARSPERLET)
            goto err;
        pvh = psh->vhdrs[nvar];
        if (pvh == _NULL)
            goto err;
        ...
        {
            pxh = pvh->xhdrs[x];
            if (tp < pxh->st || tp >= pxh->st+pxh->len)
                goto err;
            ...
            if (vect == XRMC_T_ISTEP)
                tp --;
            if (vect == XRMC_T_PSTEP)
                x  --;
            if (vect == XRMC_T_CSTEP)
            {
                x  --;
                tp --;
            }
