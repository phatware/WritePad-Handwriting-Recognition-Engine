...
#define  ST_LIKE_XR(cur)        ((cur)->code==_ST_ || ((cur)->code==_XT_ && !((cur)->other & WITH_CROSSING) ))
...
static _BOOL  LooksLikeIAndPoint( p_SPECL pPoint, _INT iPoint, _SHORT dxAbsPts, p_SHORT x, p_SHORT y );
static _BOOL  PutColonAtItsPlace( low_type _PTR low_data, p_SPECL pPt1, p_SPECL pPt2 );
static _VOID  AdjustBegEndWithoutPoint( p_SPECL pPoint );

/*  Auxiliary functions: */
static p_SPECL  SkipAnglesAndHMoves ( p_SPECL pElem );
static p_SPECL  SkipRealAnglesAndPointsAfter  ( p_SPECL pElem );
static p_SPECL  SkipRealAnglesAndPointsBefore ( p_SPECL pElem );
static p_SPECL  SkipRealAnglesAndPointsAndHMovesAfter ( p_SPECL pElem );

static _BOOL  FillBasicFeatures( p_SZD_FEATURES pszd, low_type _PTR pld );
static _BOOL  PairWorthLookingAt( p_SZD_FEATURES pszd );
static _BOOL  FillCurvFeatures( p_SZD_FEATURES pszd );
static _BOOL  LooksLikeSZ( p_SHORT x, p_SHORT y, _INT iBeg, _INT iEnd );
static _BOOL  FillComplexFeatures( p_SZD_FEATURES pszd );
static _BOOL  CheckBackDArcs( p_SZD_FEATURES pszd );
static _BOOL  CurvLikeSZ( _SHORT nCurvUp, _SHORT nCurvDown, _SHORT nMinPartCurv );
static _BOOL  CurvConsistent( p_SHORT xBuf, p_SHORT yBuf, _INT iBeg, _INT iEnd, p_SHORT ind_back );
static _BOOL  CheckSZArcs( p_SZD_FEATURES pszd );
static _BOOL  CheckDArcs( p_SZD_FEATURES pszd );
static _VOID  ArrangeAnglesNearNew( p_SZD_FEATURES pszd );
static _BOOL  IsSmthRelevant_InBetween( p_SPECL p1st, p_SPECL p2nd, _INT iBegNew, _INT iEndNew );
...
static p_SPECL  SkipAnglesAndHMoves ( p_SPECL pElem )
{
    if  ( pElem != _NULL )
    {
        ...
        while  (pElem!=_NULL && (IsAnyAngle(pElem) || pElem->code==_BSS_ ));
    }
    return  pElem;
...
static _SHORT  CurvNonQuadr( p_SHORT x, p_SHORT y, _INT iBeg, _INT iEnd )
{
...
static _BOOL  FillBasicFeatures( p_SZD_FEATURES pszd, low_type _PTR pld )
{
    ...
    if  ( pszd->i1stEnd <= pszd->i1stBeg )
    {
        /* i.e. XR fits only one real point */
        pszd->i1stEnd++;
...
static _BOOL  FillCurvFeatures( p_SZD_FEATURES pszd )
{
    ...
    if  ( iNearBegOrig >= iCenterOrig || iNearEndOrig <= iCenterOrig )
        return  _FALSE;
    ...
    pszd->nCurvUp   = CurvNonQuadr( pszd->xBuf, yBuf, iNearBegOrig, iCenterOrig );
    pszd->nCurvDown = CurvNonQuadr( pszd->xBuf, yBuf, iCenterOrig, iNearEndOrig );
...
static _BOOL  FillComplexFeatures( p_SZD_FEATURES pszd )
{
    ...
    pszd->dx   = (_SHORT)(  x[pszd->iCenter] - MEAN_OF(x[p1st_ibeg],x[p2nd_iend])
                          + SlopeShiftDx(y[pszd->iCenter] - MEAN_OF(y[p1st_ibeg],y[p2nd_iend]), pszd->pld->slope));
    /* to exclude "corner effects": */
    if  ( pszd->p1st->code == _GU_ || pszd->p1st->mark != BEG )
        pszd->iNearBeg = (_SHORT)MID_POINT(pszd->p1st);
    else
        pszd->iNearBeg = (_SHORT)(p1st_ibeg + ONE_FOURTH(pszd->iCenter-p1st_ibeg));
    if  ( pszd->p2nd->code == _GD_ || pszd->p2nd->mark != END )
    ...
    if  ( pszd->p2nd->code == _DDL_ || (   pszd->p2nd->code != _GD_ && x[p2nd_iend] > x[p2nd_ibeg] ) )
    {
        _INT    ixLeft = ixMin( p2nd_ibeg, p2nd_iend, x, y );
        if  ( pszd->iNearEnd > ixLeft && ixLeft > p2nd_ibeg  )
        {
    ...
    if  ( pszd->p1st->ipoint0 != UNDEF && pszd->iNearBeg < pszd->p1st->ipoint0 )
        pszd->iNearBeg = pszd->p1st->ipoint0;
    if  ( pszd->p2nd->ipoint0 != UNDEF && pszd->iNearEnd > pszd->p2nd->ipoint0 )
...
static _BOOL  CheckBackDArcs( p_SZD_FEATURES pszd )
{
    ...
    if  ( x[iRightUp] > x[iNearBeg] + RIGHT_EXTR_DEPTH || (   (p1st->code == _UU_ || p1st->code == _UUL_ ) && CLOCKWISE(p1st) ) )
    {
        /*10*/
        ...
        if  ( x[iRightUp] - x[iLeftMid] < RIGHT_EXTR_DEPTH && x[iRightDn] - x[iLeftMid] < RIGHT_EXTR_DEPTH )
            return  _FALSE;
        
        if  ( iRightDn < pszd->p2nd->iend - ONE_EIGHTTH(iNearEnd-iNearBeg) && iXmin_right( x, y, iRightDn, RIGHT_EXTR_DEPTH ) >= iNearEnd )
        { /*20*/
            
            _INT    diUpMid = iLeftMid - iRightUp;
            _INT    diDnMid = iRightDn - iLeftMid;
            
            if  ( diUpMid > ONE_HALF(diDnMid) && diDnMid > ONE_HALF(diUpMid) )
            {
...
static _BOOL  CurvLikeSZ( _SHORT nCurvUp, _SHORT nCurvDown, _SHORT nMinPartCurv )
{
    return  ( (nCurvUp >= nMinPartCurv && nCurvDown <= -nMinPartCurv)
             || (nCurvUp <= -nMinPartCurv && nCurvDown >= nMinPartCurv ) );
...
static _BOOL  CheckSZArcs( p_SZD_FEATURES pszd )
{
    ...
    if  (   pszd->nCurvUp < 0 && x[iNearEnd] > x[iNearBeg] )
    {
        ...
        if  ( dxEndBeg > ONE_HALF(dyEndBeg) )
            return  _FALSE;
        if  ( dxEndBeg > ONE_THIRD(dyEndBeg) || ( dxEndBeg > ONE_FOURTH(dyEndBeg)
    ...
    /* Precaution for "broken curve", like in "n": */
    {
        ...
        if  ( nCos >= MIN_COS_FOR_BROKEN_SZ )
        {
            ...
            if  ( TWO_THIRD(dxBegCenter) > dyBegCenter || TWO_THIRD(dxEndCenter) > dyEndCenter )
            {
    ...
    {
        if ( bIsStrongSZ )
            bIsStrongSZ = (_LONG)nCurvMax*MIN_CURV_FOR_SZ_RATIO > (_LONG)CURV_NORMA;
        /*
        ...
        { /*50*/
            ...
            _BOOL  bMakeSZArc;
            bMakeSZArc = ( bIsStrongSZ && THREE_HALF(HWRAbs(x[pszd->iMostFarUp] - xMeanUpCenter))
                          >= HWRAbs(y[pszd->iMostFarUp] - yMeanUpCenter) );
            bMakeSZArc = ( bMakeSZArc && THREE_HALF(HWRAbs(x[pszd->iMostFarDown] - xMeanDownCenter))
                          >= HWRAbs(y[pszd->iMostFarDown] - yMeanDownCenter) );
            bMakeSZArc = ( bMakeSZArc &&    HWRAbs(x[pszd->iMostFarUp] - x[pszd->iMostFarDown])
                          >= /*ONE_HALF*/ONE_THIRD(HWRAbs(pszd->dx)) );
            
            if  ( bMakeSZArc  &&  pszd->nCurvUp > 0 )
            {
                /*70*/
            ...
            //  else
            {
                ...
                GetTraceBox( xBuf, yBuf,
                            ...
                            &box );
                bIs1stBendable = ( ( ( bIsStrongSZ || HWRAbs(pszd->nCurvUp) > MIN_CURV_TO_BEND_ARC )
                ...
                GetTraceBox( xBuf, yBuf,
                            ...
                            pszd->i2ndEnd,
                            &box );
                bIs2ndBendable = ( ( ( bIsStrongSZ || HWRAbs(pszd->nCurvDown) > MIN_CURV_TO_BEND_ARC )
                                      && DX_RECT(box) > ONE_HALF(DY_RECT(box)) )
                                  || ( DX_RECT(box) > THREE_HALF(DY_RECT(box))
...
static _BOOL  CheckDArcs( p_SZD_FEATURES pszd )
{
    if  ( pszd->bBackDArc || (pszd->nCurvUp >= 0  &&  pszd->nCurvDown >= 0)
         || (pszd->nCurvUp <= 0  &&  pszd->nCurvDown <= 0) )
    { /*20*/
        ...
        { /*60*/
            ...
            if  ( ( dHeight >= DHEIGHT_BIG_DARCS && nCurvMin > ONE_THIRD(nCurvMax) )
                 || ( nCurvMin > 0 && ( !Is_IU_or_ID(p1st) || !Is_IU_or_ID(p2nd) ) )
                 || pszd->bBackDArc )
            {
                /* Big arc */
            ...
            if  ( !pszd->bBackDArc && (   p1st->mark == BEG || p2nd->mark == END ) )
            {
                _BOOL    bProcessOnlySloped = ( bFormula
                ...
                if  ( bProcessOnlySloped )
                {
                    ...
                    if  ( p2nd->mark != END && dxBox1 < ONE_HALF(dxBox2) )
                        b1stSloped = _FALSE;
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if  ( bIsCrossed )
                                ...
                                b1stSloped = (TWO(dyBox1) <= THREE(dxBox1) || (   dHeight > DHEIGHT_SMALL_DARCS && dyBox1 <= TWO(dxBox1) ) );
                        }
                    }
                    
                    if  (   p1st->mark != BEG && dxBox2 < ONE_THIRD(dxBox1) )
                        b2ndSloped = _FALSE;
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if  ( bIsCrossed )
                                ...
                                b2ndSloped = ( ( dyBox2 <= THREE_HALF(dxBox2)) || ((dyBox2 <= TWO(dxBox2)) && pszd->hght2nd > _DI2_ ) );
                        }
                    }
                }
                //else  {
                     ...
                     && pszd->nCurvUp >= - MIN_CURV_FOR_EMBEDDED_ARCS )
                {
                    ...
                    nExtrType = SideExtr( x, y, iNearBeg, iInEnd,
                                         ...
                                         &iSideExtr, !STRICT_ANGLE_STRUCTURE, pszd->pld->rc->lang );
                    if ( nExtrType == SIDE_EXTR_LIKE_2ND || nExtrType == SIDE_EXTR_LIKE_2ND_WEAK )
                        b1stSloped = _FALSE;
                }
...
static _VOID  ArrangeAnglesNearNew( p_SZD_FEATURES pszd )
{
    ...
    if  ( pNew != _NULL )
    {
        ...
        for  ( pAngle=pNew->next; pAngle!=_NULL && IsAnyAngle(pAngle); pAngle=pAngle->next )
        {
            if  ( pAngle->iend <= iFrstPostAngles )
            {
                p_SPECL pContinue = pAngle;
                if  (   pAngle->ibeg <= iLastPrevAngles
                     || (   ( pNew->code == _TZ_  ||  pNew->code == _TS_ )
                         && (   pAngle->ibeg <= pszd->iCenter || pAngle->mark != ANGLE ) ) )
                {
...
_SHORT  FindDArcs ( low_type _PTR pld )
{
    ...
    for  ( szd.p1st=specl->next; szd.p1st!=_NULL; szd.p1st=szd.p1st->next )
    { /*10*/
...
#if  NEW_VERSION
#else
            if  ( szd.y[szd.p1st->ibeg-1] == BREAK || szd.y[szd.p2nd->iend+1] == BREAK )
#endif
            { /*30*/
                ...
                if  (   PairWorthLookingAt(&szd) && FillComplexFeatures(&szd) )
                {
                    if  (   CheckBackDArcs(&szd) || !CheckSZArcs(&szd) )
                        CheckDArcs(&szd);
                }
...
_VOID  Adjust_I_U ( low_type _PTR low_data )  /*CHE*/
{
    ...
    for  ( cur = specl->next;cur != _NULL  &&  cur->next != _NULL;cur = cur->next )
    { /*10*/
        ...
        if  (prv->iend >= cur->ibeg|| nxt->ibeg <= cur->iend)
        ...
        if  ( nUpDown != NO_DIR )
        {
            ...
            _SHORT  nCurv1     = CurvMeasure( x, y, MID_POINT(prv),iMidCur, iCurv1Pt );
            _SHORT  nCurv2     = CurvMeasure( x, y, iMidCur,MID_POINT(nxt), iCurv2Pt );
...
_BOOL  FindSideExtr( low_type _PTR low_data )
{
    ...
    for  ( p1st=specl->next; p1st!=_NULL; p1st=p2nd )
    {
        /*10*/
        ...
        if  ( (p2nd = p1st->next /*SkipAnglesAfter(p1st)*/) == _NULL )
            break;
        if  ( !(p1st->mark==MINW && p2nd->mark==MAXW) && !(p1st->mark==MAXW && p2nd->mark==MINW) )
            continue;
        ...
        nExtrType = SideExtr( x, y, iMid1st, iMid2nd,
                             ...
                             &iSideExtr, STRICT_ANGLE_STRUCTURE, low_data->rc->lang );
        ...
        else  if  ( nExtrType == SIDE_EXTR_LIKE_2ND || nExtrType == SIDE_EXTR_LIKE_2ND_WEAK )
        {
            if  ( p2nd->next != _NULL && p2nd->next->mark != END )
                continue;
...
_BOOL  PostFindSideExtr( low_type _PTR low_data )
{
    ...
    { /*10*/
        ...
        if  ( IsUpperElem(p1st) && IsLowerElem(p2nd) )
        {
            if ((lang != LANGUAGE_GERMAN && lang != LANGUAGE_FRENCH
                 && lang != LANGUAGE_ITALIAN && lang != LANGUAGE_PORTUGUESE)
                && (p1st->code==_DUR_ || p1st->code==_CUR_))
        ...
        else  if  ( IsLowerElem(p1st) && ( IsUpperElem(p2nd)  ||  p2nd->code == _Gr_ ) )
        {
            if ((lang != LANGUAGE_GERMAN && lang != LANGUAGE_FRENCH
                 && lang != LANGUAGE_ITALIAN)
                && (p2nd->code==_DUL_ || p2nd->code==_CUL_))
                continue;  /* Don't consider back-moving upper elems. */
            ...
            if ((lang == LANGUAGE_GERMAN || lang == LANGUAGE_FRENCH
                 || lang == LANGUAGE_ITALIAN || lang == LANGUAGE_PORTUGUESE)
                && HEIGHT_OF(p1st) <= _DI1_ )  /* It's hanging */
                bWeakNewElem = _TRUE;
        }
        else
            continue;
        ...
        else
        {
            ...
            if  ( iExtr > iRef1st )  {
                if  ( p1st->code==_UU_ || p1st->code==_UUL_ || p1st->code==_UUR_ )
                {
                    ...
                    if  ( dx1stBegEnd < ONE_HALF(dy1stBegExtr) || dx1stBegEnd < ONE_HALF(dy1stEndExtr) )
        ...
        else
        {
            ...
            if  ( iExtr < iRef2nd )
            {
                if  ( p2nd->code==_UU_ || p2nd->code==_UUL_ || p2nd->code==_UUR_ )
                {
                    ...
                    if  ( dx2ndBegEnd < ONE_HALF(dy2ndBegExtr) || dx2ndBegEnd < ONE_HALF(dy2ndEndExtr) )
        ...
        nExtrType = SideExtr( x, y, iRef1st, iRef2nd,
                             ...
                             &iSideExtr, !STRICT_ANGLE_STRUCTURE, lang );
        ...
        if  (   !b1stUpper
             ...
             && p2nd->code != _UUC_
             && (   p2nd->next == _NULL || p2nd->next->code != _CDL_ ) )
        {
            /*20*/
            ...
            if  (nDist1 >= ONE_THIRD(nDist2) && nDist2 >= ONE_THIRD(nDist1)
                 && CurvMeasure ( x, y, iRef1st, iRef2nd, iSideExtr ) > 0 )
            {
                _INT  nCos;
                _INT  iMid1 = MEAN_OF( iRef1st, iSideExtr );
                
                if  ( Distance8( x[iMid1],y[iMid1], x[iSideExtr],y[iSideExtr] ) > ONE_FOURTH(nDist2) )
                    nCos = (_INT)cos_vect( iSideExtr, iMid1, iSideExtr, iRef2nd, x, y );
                else
                    nCos = (_INT)cos_vect( iSideExtr, iRef1st, iSideExtr, iRef2nd, x, y );
        ...
        if  ( !b1stUpper
             && (nExtrType == NO_SIDE_EXTR || nExtrType == SIDE_EXTR_LIKE_2ND || nExtrType == SIDE_EXTR_LIKE_2ND_WEAK )
             && (p1st->code == _ID_ || p1st->code == _UD_ || p1st->code == _UDC_ )
             && COUNTERCLOCKWISE( p1st )
             && (CLOCKWISE( p2nd ) || nExtrType != NO_SIDE_EXTR )
             && p2nd->code != _CUR_
             && p2nd->mark != END
             && p2nd->next != _NULL
             && p2nd->next->mark != END )
        { /*25*/
            ...
            if  ( pPrev!=specl && pPrev!=_NULL && IsUpperElem(pPrev)
                 && (pPrev->mark == BEG || pPrev->prev == _NULL || pPrev->prev == specl || IsAnyBreak(pPrev->prev) ) )
            {
                ...
                { /*27*/
                    ...
                    if  ( iRefForO > 0
                         ...
                         && (   dyO > TWO_THIRD(yDown1st - y[iUp2nd]) || nExtrType != NO_SIDE_EXTR ) )
                    {
                        _SHORT  nRetCod;
                        ...
                        if  ( nRetCod == RETC_OK )
                        {
                            ...
                            _INT  dxNeck = x[iRefForO] - x[iRightPrev] + SlopeShiftDx( (y[iRefForO] - y[iRightPrev]), low_data->slope );
...
#define  MIN_CURV_LANG_GERMAN_er  2
#define  MIN_COS_FOR_SHARP_er    55
        
        if ((lang == LANGUAGE_GERMAN || lang == LANGUAGE_FRENCH
             || lang == LANGUAGE_ITALIAN || lang == LANGUAGE_PORTUGUESE)
            && (nExtrType == NO_SIDE_EXTR || nExtrType == SIDE_EXTR_TRACE_FOR_er))
        {
            /*30*/
            ...
            if  ( !b1stUpper )
            {
                ...
                if  ( p2nd->code == _GU_ || p2nd->code == _Gr_ || p2nd->code == _GUs_ )
                    bMake_er = _TRUE;
                
                if  ( !bMake_er
                     && ( (p1st->code == _ID_ && (   p1st->mark == STICK || COUNTERCLOCKWISE(p1st) ) )
                         || (p1st->code == _UD_ && COUNTERCLOCKWISE(p1st) )
                         || p1st->code == _DDR_
                         || p1st->code == _DDL_
                         || p1st->code == _GDs_
                         || p1st->code == _Gr_ ) )
                ...
                {
                    if  (   CurvMeasure( x, y, iRef1st, iRef2nd, iSideExtr ) < MIN_CURV_LANG_GERMAN_er
                         && CurvMeasure( x, y, MEAN_OF(iRef1st,iSideExtr), iRef2nd, iSideExtr ) < MIN_CURV_LANG_GERMAN_er )
                    {
                        p_SPECL  pPrev = SkipRealAnglesAndPointsBefore ( p1st );
                        
                        if  (   pPrev!=specl && pPrev!=_NULL && ( IsUpperElem(pPrev) || pPrev->code==_BSS_ ) )
                        {
...
#undef  MIN_CURV_LANG_GERMAN_er
        
        if ((lang != LANGUAGE_GERMAN && lang != LANGUAGE_FRENCH
             && lang != LANGUAGE_ITALIAN && lang != LANGUAGE_PORTUGUESE)
            && !b1stUpper && p2nd->code == _CUR_ )
        {
            if  ( nExtrType == SIDE_EXTR_LIKE_2ND || nExtrType == SIDE_EXTR_LIKE_2ND_WEAK )
            {
                ...
                if  ( pAfter != _NULL )
                {
                    ...
                    if  ( (p2nd->mark==CROSS || p2nd->mark==STICK) && iSideExtr > (p2nd+1)->ibeg )
                        nExtrType = NO_SIDE_EXTR;
                }
            }
        }
        ...
        if  ( b1stUpper && (nExtrType == SIDE_EXTR_LIKE_1ST || nExtrType == SIDE_EXTR_LIKE_1ST_WEAK ) )
        {
            if  ( nExtrType == SIDE_EXTR_LIKE_1ST_WEAK || p1st->prev == specl || IsAnyBreak(p1st->prev) )
            ...
            if  ((lang == LANGUAGE_GERMAN || lang == LANGUAGE_FRENCH
                  || lang == LANGUAGE_ITALIAN || lang == LANGUAGE_PORTUGUESE)
                 && (pCheck == _NULL || IsAnyBreak(pCheck) || IsXTorST(pCheck)))
                bWeakNewElem = _TRUE;
            iBegNew = ONE_THIRD(iRef1st + TWO(iSideExtr));
            iEndNew = iSideExtr;
            if  ( bWeakNewElem && IsSmthRelevant_InBetween( p1st, p2nd, iBegNew, iEndNew ) )
                continue; /* Don't mark weak elem if there already is something. */
            if  ( IsLowerElem(pCheck) )
            {
                dyExtrCheck = y[iYdown_range(y,pCheck->ibeg,pCheck->iend)] - y[iYup_range(y,p1st->ibeg,p1st->iend)];
                if  ( HWRAbs(dyExtrCheck) < MIN_DY_MAKE_NEW )
                    continue; /* Don't make new elem. in low depth environment */
            }
            if  ( !bSpecialCase && ( ClosedSquare( x, y, iRef1st, iRef2nd, &SqRetCod ) <= 0 || SqRetCod != RETC_OK ) )
        ...
        else  if  ((lang == LANGUAGE_GERMAN || lang == LANGUAGE_FRENCH
                    || lang == LANGUAGE_ITALIAN || lang == LANGUAGE_PORTUGUESE)
                   && !b1stUpper
                   ...
                   && x[iSideExtr] > x[iRef1st])
        {
            if  ( (pNew = NewSPECLElem(low_data)) != _NULL )
            {
                        ...
                        && pNew->next->ibeg < pNew->ibeg )
                {
                   ...
                   && ( nExtrType == SIDE_EXTR_LIKE_2ND || nExtrType == SIDE_EXTR_LIKE_2ND_WEAK )
        ...
        if  ( bMadeNewElem )
        {
            ...
            if  ( bWeakNewElem )
            {
                ...
                {
                    if  ( nExtrType == SIDE_EXTR_LIKE_1ST || nExtrType == SIDE_EXTR_LIKE_2ND )
...
#define  IsAnyRealAngle(pElem)    (   REF(pElem)->code==_ANl || REF(pElem)->code==_ANr  )
...
static p_SPECL  SkipRealAnglesAndPointsAfter ( p_SPECL pElem )
{
    ...
    if  ( pElem != _NULL )
    {
        ...
        while  ( pElem!=_NULL && ( IsAnyRealAngle(pElem) || IsXTorST(pElem) ) );
    }
    return  pElem;
...
static p_SPECL  SkipRealAnglesAndPointsAndHMovesAfter ( p_SPECL pElem )
{
    ...
    if  ( pElem != _NULL )
    {
        ...
        while  ( pElem!=_NULL && ( IsAnyRealAngle(pElem) || IsXTorST(pElem) || pElem->code==_BSS_ ) );
    }
...
static _BOOL IsSmthRelevant_InBetween( p_SPECL p1st, p_SPECL p2nd, _INT iBegNew, _INT iEndNew )
{
    ...
    for  ( p1st=p1st->next; p1st!=_NULL  &&  p1st!=p2nd; p1st=p1st->next )
    {
        if  ( !IsXTorST( p1st )  &&  p1st->code != _BSS_ )
        {
            if  ( IsAnyRealAngle( p1st ) )
            {
                if  (   p1st->ibeg > iEndNew || p1st->iend < iBegNew )
...
_SHORT  RestoreColons( low_type _PTR low_data )
{
    ...
    for ( pCur=low_data->specl->next, nPts=0; pCur != _NULL; pCur = pCur->next
         )
    { /*15*/
        ...
        { /*10*/
            ...
            {
                for  ( pAnchor=pCur->prev; pAnchor!=_NULL && !IsUpperElem(pAnchor); pAnchor=pAnchor->prev )
                    ;
            }
            else
            {
                for  ( pAnchor=pCur->next; pAnchor!=_NULL && ST_LIKE_XR(pAnchor); pAnchor=pAnchor->next )
                    ;
            }
            ...
            if  ( pts[nPts].ptr->code == _XT_ )
            { //make sure _XT_ isn't attached to something
                _SHORT  xMin, xMax;
                if  ( pts[nPts].dx2Extr < ONE_HALF(STD_BASELINE_HEIGHT) )
                    continue;  //too close to corresponding xr
                if  ( !xMinMax ( pts[nPts].ptr->ibeg, pts[nPts].ptr->iend, x, y, &xMin, &xMax ) )
    ...
    for  ( i=0;  i<nPts-1;  i++ )
    {
        ...
        for  ( j=i+1;  j<nPts;  j++ )  { /*40*/
            ...
            if  ( pt_j->iend != pt_i->ibeg-2 && pt_i->iend != pt_j->ibeg-2 )
            ...
            if  ( dyAbs < ONE_FOURTH(STD_BASELINE_HEIGHT) || dyAbs > TWO(STD_BASELINE_HEIGHT) )
...
static _BOOL  LooksLikeIAndPoint( p_SPECL pPoint, _INT iPoint, _SHORT dxAbsPts, p_SHORT x, p_SHORT y )
{
    ...
    if  ( (pNxt=pPoint->next) != _NULL && IsUpperElem(pNxt) )
    {
        ...
        if  ( x[iPoint] < x[iPt_nxt] )
            bRet = _TRUE;
        else  if  ( y[iPoint] < y[iPt_nxt] && dxAbsPts > (x[iPoint] - x[iPt_nxt]) )
            bRet = _TRUE;
    }
...
static _BOOL  PutColonAtItsPlace( low_type _PTR low_data, p_SPECL pPt1, p_SPECL pPt2 )
{
    ...
    for  ( pCur=low_data->specl->next, p1st=_NULL, pLast=_NULL; pCur!=_NULL; pCur=pCur->next )
    {
    ...
    for  ( pCur=p1st, dxBest=ALEF, pBest=_NULL, bWasDxNegative=bWasDxPositive=_FALSE; pCur!=_NULL; pCur=pCur->next )
    {
        /*20*/
        
        if  (   IsAnyBreak(pCur) || pCur == p1st || pCur == pLast )
        {
            ...
            else
            {
                ...
                if  ( IsAnyBreak( pCur ) )
                {
                    ...
                    {     /* Unite breaks around these points, if any */
                        if  ( IsAnyBreak(pPt2->next) && pPt2->next->next != _NULL )
    ...
    bWasXTST = _FALSE;      
    for  ( pCur=pBest->prev; pCur!=NULL && pCur!=low_data->specl; pCur=pCur->prev )
    {
        ...
        if  ( pCur->ibeg < pPt1->ibeg || pCur->ibeg < pPt2->ibeg )
            break;
    ...
    if  ( !IsAnyBreak(pBest) )
    {
        ...
        if  ( pBest == pLast )
        {
            /* i.e. if the last non-point-like elem in specl */
            pNewBrk->ibeg =
        ...
        {
            /* i.e. the 1st non-point-like elem in specl */
            pNewBrk->ibeg =
    ...
    for  ( pCur=low_data->specl->next; pCur!=_NULL; pCur=pCur->next )
    { /*50*/
        if  ( IsAnyBreak( pCur )
             && pCur->next != _NULL
